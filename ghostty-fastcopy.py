...
PATTERNS: list[tuple[str, str]] = [
    ("url",         r"(?i)\b(?:https?|ftp|file)://[-A-Z0-9+&@#/%?=~_|!:,.;]*[-A-Z0-9+&@#/%=~_|]"),
    ("ipv6",        r"\b(?:[0-9a-fA-F]{1,4}:){7}(?:[0-9a-fA-F]{1,4}|:)\b"),
    ("uuid",        r"(?i)\b[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}\b"),
    ("email",       r"(?i)\b[A-Z0-9._%+\-]+@[A-Z0-9.\-]+\.[A-Z]{2,}\b"),
    ("macaddr",     r"(?i)\b(?:[0-9A-F]{2}[:\-]){5}[0-9A-F]{2}\b"),
    ("k8s",         r"\b[a-z0-9](?:[a-z0-9\-]*[a-z0-9])?-[a-z0-9]{8,10}-[a-z0-9]{5}\b"),
    ("rust_path",   r"\b[a-z_][a-z0-9_]*(?:::[a-z_][a-z0-9_]*)+\b"),
    ("path",        r"(?:(?:^|(?<!\w))(?:~(?=/)|/)[\w./\-~@%+=:,]+)"),
    ("ipv4",        r"\b(?:\d{1,3}\.){3}\d{1,3}(?::\d{1,5})?\b"),
    ("git_hash",    r"(?<![0-9a-f\-])[0-9a-f]{7,40}(?![0-9a-f])"),
